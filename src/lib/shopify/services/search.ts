...
export interface SearchParams {
  ...
  sortKey?: string;
  reverse?: boolean;
  /** Canonical Shopify ProductFilter inputs (faceted filtering). */
  filters?: unknown[];
}

export interface SearchResult extends Paginated<ProductCard> {
  totalCount: number;
  /** Available facets for the current result set (same shape as collection filters). */
  filters: CollectionFilter[];
}

/**
 * Full product search results page. With an empty `query` ("") this returns
 * the entire catalogue plus its faceted filters — used by the all-products
 * PLP (the top-level `products` query supports neither filter input nor
 * facet output; only `collection.products` and `search` do).
 */
